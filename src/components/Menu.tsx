...
export default function Component({ role }: MenuProps) {
  ...
  const renderMenuItem = (item: typeof visibleItems[0], index: number) => {
    ...
    return (
      <li key={item.label} className={index >= 3 ? "lg:hidden" : ""}>
        <Link
          href={item.href}
          className="flex flex-col items-center justify-center gap-1 p-2 text-gray-500 hover:bg-card transition-colors duration-200 rounded-md"
                ...
                .map((item) => {
                  ...
                  return (
                    <li key={item.label}>
                      <Link
                        href={item.href}
                        className="flex items-center justify-start gap-4 text-gray-500 py-2 px-3 rounded-md w-full transition-colors duration-300 ease-in-out hover:text-black dark:hover:text-white hover:animate-out"
                        aria-label={item.label}
                      >
